...
from .models import Attendance, Patient,Employee
...
def identify_faces(video_capture):
    ...
    while True:
        ...
        if process_this_frame:
            predictions = predict(rgb_frame, model_path="c360app/facerec/models/trained_model.clf")
            # print(predictions)
...
def click_photos(request, emp_id):
	...
	click(emp.firstname, emp.id, cam)
...
def detected(request):
	if request.method == 'GET':
		...
		det_list = Attendance.objects.filter(time_stamp__date=date_formatted).order_by('time_stamp').reverse()
...
def identify(request):
	...
	return HttpResponseRedirect(reverse('emplogin'))
